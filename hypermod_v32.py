...
class HyperModEngineV32AuroraConnected:
    ...
    def aplicar_fases(self, audio_data: np.ndarray, configuracion: Dict[str, Any]) -> np.ndarray:
        """
        ✅ MÉTODO FALTANTE: Aplica secuencia de fases al audio
        
        Args:
            audio_data: Array de audio a procesar
            configuracion: Configuración de fases a aplicar
            
        Returns:
            Audio con fases aplicadas
        """
        try:
            # Usar la lógica existente del motor HyperMod
            if hasattr(self, '_aplicar_estructura_fases'):
                # Si tiene método interno, usarlo
                return self._aplicar_estructura_fases(audio_data, configuracion)
            
            elif hasattr(self, 'generar_audio'):
                # Regenerar con configuración específica de fases
                duracion = len(audio_data) / self.sample_rate if hasattr(self, 'sample_rate') else len(audio_data) / 44100
                config_fases = {
                    'duracion': duracion,
                    'fases': configuracion.get('fases', ['intro', 'desarrollo', 'climax', 'outro']),
                    'objetivo': configuracion.get('objetivo', 'estructura_completa')
                }
                
                # Generar nueva estructura con fases
                resultado = self.generar_audio(config_fases, duracion)
                
                if isinstance(resultado, dict) and 'audio_data' in resultado:
                    return resultado['audio_data']
                elif isinstance(resultado, np.ndarray):
                    return resultado
                else:
                    # Fallback: aplicar envelope básico
                    return self._aplicar_envelope_fases(audio_data, configuracion)
            
            else:
                # Fallback básico: aplicar envelope de fases
                return self._aplicar_envelope_fases(audio_data, configuracion)
                
        except Exception as e:
            self.logger.error(f"❌ Error en aplicar_fases: {e}") if hasattr(self, 'logger') else None
            return audio_data  # Retornar audio original si hay error

    def crear_estructura_capas(self, configuracion: Dict[str, Any]) -> Dict[str, Any]:
        """
        ✅ MÉTODO FALTANTE: Crea estructura de capas según configuración
        
        Args:
            configuracion: Configuración para crear estructura
            
        Returns:
            Diccionario con estructura de capas creada
        """
        try:
            # Extraer parámetros básicos
            duracion = configuracion.get('duracion', 300.0)
            objetivo = configuracion.get('objetivo', 'estructura_completa')
            intensidad = configuracion.get('intensidad', 0.7)
            
            # Crear estructura básica de capas
            estructura = {
                'capas_base': [],
                'capas_estructurales': [],
                'fases_temporales': [],
                'metadata': {
                    ...
                    'tipo_estructura': 'hypermod_v32'
                }
            }
            
            # Si hay método interno de capas, usarlo
            if hasattr(self, '_crear_capas_estructurales'):
                capas_internas = self._crear_capas_estructurales(configuracion)
                if capas_internas:
                    estructura['capas_estructurales'] = capas_internas
            
            # Crear fases temporales basadas en duración
            num_fases = max(3, int(duracion / 60))  # Al menos 3 fases, 1 por minuto
            for i in range(num_fases):
                fase_inicio = (duracion / num_fases) * i
                fase_duracion = duracion / num_fases
                
                fase = {
                    'nombre': f'fase_{i+1}',
                    'inicio': fase_inicio,
                    'duracion': fase_duracion,
                    'intensidad_relativa': self._calcular_intensidad_fase(i, num_fases, intensidad),
                    'tipo': self._determinar_tipo_fase(i, num_fases)
                }
                estructura['fases_temporales'].append(fase)
            
            # Crear capas base según objetivo
            if objetivo in ['enfoque', 'claridad_mental']:
                estructura['capas_base'] = self._crear_capas_cognitivas(configuracion)
            elif objetivo in ['relajacion', 'meditacion']:
                estructura['capas_base'] = self._crear_capas_relajacion(configuracion)
            else:
                estructura['capas_base'] = self._crear_capas_genericas(configuracion)
            
            return estructura
            
        except Exception as e:
            self.logger.error(f"❌ Error en crear_estructura_capas: {e}") if hasattr(self, 'logger') else None
            # Estructura mínima de fallback
            return {
                'capas_base': [{'tipo': 'base', 'frecuencia': 440, 'amplitud': 0.5}],
                'capas_estructurales': [],
                'fases_temporales': [{'nombre': 'unica', 'inicio': 0, 'duracion': configuracion.get('duracion', 300)}],
                'metadata': {'tipo_estructura': 'fallback', 'error': str(e)}
            }

    # ===============================================================================
    # MÉTODOS AUXILIARES PARA HYPERMOD
    # ===============================================================================

    def _aplicar_envelope_fases(self, audio_data: np.ndarray, configuracion: Dict[str, Any]) -> np.ndarray:
        """Aplica envelope básico simulando fases"""
        try:
            fases = configuracion.get('fases', ['intro', 'desarrollo', 'outro'])
            num_fases = len(fases)
            ...
            samples_por_fase = len(audio_data) // num_fases
            
            for i, fase in enumerate(fases):
                inicio = i * samples_por_fase
                fin = (i + 1) * samples_por_fase if i < num_fases - 1 else len(audio_data)
                
                # Aplicar envelope según tipo de fase
                if fase in ['intro', 'entrada']:
                    # Fade in gradual
                    envelope = np.linspace(0.1, 1.0, fin - inicio)
                elif fase in ['outro', 'salida', 'final']:
                    # Fade out gradual
                    envelope = np.linspace(1.0, 0.1, fin - inicio)
                elif fase in ['climax', 'pico']:
                    # Intensidad máxima
                    envelope = np.ones(fin - inicio) * 1.0
                else:
                    # Desarrollo normal
                    envelope = np.ones(fin - inicio) * 0.8
                
                # Aplicar envelope
                if audio_data.ndim == 2:
                    audio_procesado[inicio:fin] *= envelope[:, np.newaxis]
                else:
                    audio_procesado[inicio:fin] *= envelope
    ...
    def _calcular_intensidad_fase(self, indice_fase: int, total_fases: int, intensidad_base: float) -> float:
        """Calcula intensidad relativa para cada fase"""
        if total_fases <= 1:
            return intensidad_base
        
        # Curva de intensidad: sube hasta el medio, luego baja
        posicion_relativa = indice_fase / (total_fases - 1)
        
        if posicion_relativa <= 0.5:
            # Primera mitad: intensidad creciente
            factor = 0.5 + posicion_relativa
        else:
            # Segunda mitad: intensidad decreciente
            factor = 1.5 - posicion_relativa
        
        return min(1.0, intensidad_base * factor)

    def _determinar_tipo_fase(self, indice_fase: int, total_fases: int) -> str:
        """Determina el tipo de fase según posición"""
        if indice_fase == 0:
            return 'introduccion'
        elif indice_fase == total_fases - 1:
            return 'conclusion'
        elif indice_fase == total_fases // 2:
            return 'climax'
        else:
            return 'desarrollo'
    
    def aplicar_fases(self, audio_base, secuencia_fases, config=None):
        start_time = time.time()
        
        if not isinstance(audio_base, np.ndarray):
            raise ValueError(f"audio_base debe ser numpy.ndarray, recibido: {type(audio_base)}")
        
        if audio_base.size == 0:
            raise ValueError("audio_base no puede estar vacío")
        
        # PREPARAR CONFIGURACIÓN REAL
        config = config or {}
        config_hypermod = {
            'duration': config.get('duracion_total', len(audio_base) / self.sample_rate),
            'base_frequency': config.get('frecuencia_base', 10.0),
            'intensity': config.get('intensidad_global', 0.7),
            'secuencia_fases': secuencia_fases
        }
        
        try:
            # USAR GESTOR REAL PARA CREAR LAYERS DESDE SECUENCIA
            logger.info(f"🎛️ Aplicando fases usando GestorAuroraIntegradoV32 real")
            layers_fases = self.gestor_integrado.crear_layers_desde_secuencia_fases(
                secuencia_fases, config_hypermod
            )
            
            if not layers_fases:
                raise RuntimeError("GestorAuroraIntegradoV32.crear_layers_desde_secuencia_fases no generó layers")
            
            # GENERAR AUDIO DE FASES CON NEURO WAVE GENERATOR REAL
            logger.info(f"🌊 Generando ondas con NeuroWaveGenerator real - {len(layers_fases)} layers")
            audio_fases = np.zeros_like(audio_base)
            fases_aplicadas = []
            
            for i, layer_config in enumerate(layers_fases):
                try:
                    # USAR WAVE GENERATOR REAL
                    onda_fase = self.wave_generator.generate_wave(layer_config, self.sample_rate)
                    
                    if onda_fase is None or len(onda_fase) == 0:
                        raise RuntimeError(f"NeuroWaveGenerator.generate_wave falló para layer {i}")
                    
                    # APLICAR MODULACIÓN REAL SI ESPECIFICADA
                    if layer_config.modulation:
                        onda_fase = self.wave_generator.apply_modulation(
                            onda_fase, layer_config.modulation, layer_config.frequency
                        )
                    
                    # APLICAR EFECTOS ESPACIALES REALES SI HAY POSICIÓN
                    if layer_config.spatial_position:
                        onda_fase = self.wave_generator.apply_spatial_effects(
                            onda_fase, layer_config.spatial_position
                        )
                    
                    # AJUSTAR DURACIÓN AL AUDIO BASE
                    if len(onda_fase) != len(audio_base):
                        if len(onda_fase) > len(audio_base):
                            onda_fase = onda_fase[:len(audio_base)]
                        else:
                            # Repetir si es más corta
                            repeticiones = (len(audio_base) // len(onda_fase)) + 1
                            onda_fase = np.tile(onda_fase, repeticiones)[:len(audio_base)]
                    
                    # MEZCLAR CON AUDIO DE FASES
                    if len(audio_fases.shape) == 1:
                        audio_fases += onda_fase * layer_config.amplitude
                    else:
                        # Estéreo: aplicar con ligera variación entre canales
                        if len(audio_fases.shape) == 2:
                            for canal in range(audio_fases.shape[0]):
                                factor_canal = 1.0 + (canal * 0.05)  # Ligera variación
                                if canal < len(audio_fases):
                                    audio_fases[canal] += onda_fase * layer_config.amplitude * factor_canal
                    
                    # REGISTRAR FASE APLICADA
                    fases_aplicadas.append({
                        "layer_index": i,
                        "frequency": layer_config.frequency,
                        "amplitude": layer_config.amplitude,
                        "wave_type": layer_config.wave_type.value,
                        "duration": layer_config.duration,
                        "modulation": layer_config.modulation.value if layer_config.modulation else None,
                        "samples_generados": len(onda_fase),
                        "motor_real_usado": "NeuroWaveGenerator"
                    })
                    
                except Exception as e:
                    error_msg = f"Error procesando layer {i} con NeuroWaveGenerator real: {e}"
                    logger.error(f"❌ {error_msg}")
                    raise RuntimeError(error_msg)
            
            # COMBINAR AUDIO BASE + FASES CON MOTOR REAL
            logger.info(f"🔄 Combinando audio base + {len(fases_aplicadas)} fases reales")
            audio_final = audio_base + audio_fases
            
            # NORMALIZACIÓN USANDO ESTADÍSTICAS REALES DEL MOTOR
            max_val = np.max(np.abs(audio_final))
            if max_val > 0.95:
                factor_normalizacion = 0.95 / max_val
                audio_final = audio_final * factor_normalizacion
                logger.info(f"🔧 Normalización aplicada: factor {factor_normalizacion:.3f}")
            
            # ACTUALIZAR ESTADÍSTICAS REALES DEL MOTOR
            processing_time = time.time() - start_time
            self.stats.layers_generados += len(layers_fases)
            self.stats.tiempo_total_generacion += processing_time
            self.stats.ultima_actualizacion = datetime.now()
            
            # RESULTADO CON METADATOS REALES
            resultado = {
                "audio_data": audio_final,
                "sample_rate": self.sample_rate,
                "duracion_sec": len(audio_final) / self.sample_rate,
                "fases_aplicadas": fases_aplicadas,
                "numero_fases": len(fases_aplicadas),
                "layers_generados": len(layers_fases),
                "metadatos": {
                    "motor": self.nombre,
                    "version": self.version,
                    "metodo": "aplicar_fases",
                    "motor_real_usado": "GestorAuroraIntegradoV32 + NeuroWaveGenerator",
                    "tiempo_procesamiento": processing_time,
                    "layers_config_reales": [
                        {
                            "freq": layer.frequency,
                            "amp": layer.amplitude,
                            "type": layer.wave_type.value,
                            "dur": layer.duration
                        } for layer in layers_fases
                    ],
                    "estadisticas_motor": {
                        "total_layers_generados": self.stats.layers_generados,
                        "tiempo_total_motor": self.stats.tiempo_total_generacion
                    }
                }
            }
            
            logger.info(f"🎛️ FASES APLICADAS EXITOSAMENTE: {len(fases_aplicadas)} fases reales en {processing_time:.2f}s")
            return resultado
            
        except Exception as e:
            error_msg = f"Error crítico aplicando fases con motores reales: {e}"
            logger.error(f"❌ {error_msg}")
            # NO FALLBACK SILENCIOSO - REPORTAR ERROR CLARAMENTE
            raise RuntimeError(f"aplicar_fases FALLÓ usando infraestructura real: {error_msg}")
    
    def crear_estructura_capas(self, objetivo="relajacion", duracion_sec=300.0, config=None):
        start_time = time.time()
        
        if duracion_sec <= 0:
            raise ValueError(f"duracion_sec debe ser positiva, recibido: {duracion_sec}")
        
        if duracion_sec > 3600:
            raise ValueError(f"duracion_sec máxima es 3600s (1h), recibido: {duracion_sec}")
        
        # CONFIGURACIÓN REAL PARA MOTOR
        config = config or {}
        config_hypermod = {
            'objetivo': objetivo,
        ...
        config_hypermod.update(config)
        
        try:
            # DETECTAR TEMPLATES REALES USANDO DETECTOR DE COMPONENTES
            logger.info(f"🔍 Detectando templates reales para objetivo '{objetivo}'")
            componentes_detectados = self.gestor_integrado._obtener_componentes()
            
            if not componentes_detectados:
                raise RuntimeError("DetectorComponentesHyperMod no detectó componentes")
            
            templates_reales = componentes_detectados.get('templates', {})
            if not templates_reales:
                raise RuntimeError("No se detectaron templates reales en el sistema")
            
            # USAR GESTOR REAL PARA CREAR LAYERS DESDE TEMPLATE
            logger.info(f"🏗️ Creando estructura usando GestorAuroraIntegradoV32 real")
            layers_estructura = self.gestor_integrado.crear_layers_desde_template_objetivo(
                objetivo, config_hypermod
            )
            
            if not layers_estructura:
                raise RuntimeError(f"GestorAuroraIntegradoV32.crear_layers_desde_template_objetivo falló para objetivo '{objetivo}'")
            
            logger.info(f"✅ Gestor real generó {len(layers_estructura)} layers para '{objetivo}'")
            
            # GENERAR AUDIO COMPLETO CON WAVE GENERATOR REAL
            num_samples = int(duracion_sec * self.sample_rate)
            audio_estructura = np.zeros((2, num_samples))  # Estéreo
            capas_generadas = []
            
            for i, layer_config in enumerate(layers_estructura):
                try:
                    logger.debug(f"🌊 Generando layer {i+1}/{len(layers_estructura)} con NeuroWaveGenerator real")
                    
                    # GENERAR ONDA CON MOTOR REAL
                    onda_layer = self.wave_generator.generate_wave(layer_config, self.sample_rate)
                    
                    if onda_layer is None:
                        raise RuntimeError(f"NeuroWaveGenerator.generate_wave retornó None para layer {i}")
                    
                    if len(onda_layer) == 0:
                        raise RuntimeError(f"NeuroWaveGenerator.generate_wave retornó array vacío para layer {i}")
                    
                    # APLICAR MODULACIÓN REAL SI ESTÁ CONFIGURADA
                    if layer_config.modulation:
                        logger.debug(f"🔧 Aplicando modulación real: {layer_config.modulation.value}")
                        onda_layer = self.wave_generator.apply_modulation(
                            onda_layer, layer_config.modulation, layer_config.frequency
                        )
                    
                    # CALCULAR POSICIÓN TEMPORAL EN LA ESTRUCTURA
                    inicio_temporal = i / len(layers_estructura)
                    fin_temporal = (i + 1) / len(layers_estructura)
                    
                    inicio_sample = int(inicio_temporal * num_samples)
                    fin_sample = int(fin_temporal * num_samples)
                    duracion_layer = fin_sample - inicio_sample
                    
                    # AJUSTAR ONDA A DURACIÓN DE LAYER
                    if len(onda_layer) != duracion_layer:
                        if len(onda_layer) > duracion_layer:
                            onda_layer = onda_layer[:duracion_layer]
                        else:
                            # Repetir para llenar duración
                            repeticiones = (duracion_layer // len(onda_layer)) + 1
                            onda_layer = np.tile(onda_layer, repeticiones)[:duracion_layer]
                    
                    # APLICAR ESPACIALIZACIÓN REAL
                    if layer_config.spatial_position:
                        logger.debug(f"🎯 Aplicando efectos espaciales reales")
                        onda_layer = self.wave_generator.apply_spatial_effects(
                            onda_layer, layer_config.spatial_position
                        )
                    
                    # COLOCAR EN ESTRUCTURA ESTÉREO CON BALANCE REAL
                    balance_factor = inicio_temporal  # 0.0 (izq) a 1.0 (der)
                    factor_izq = np.sqrt(1 - balance_factor) * layer_config.amplitude
                    factor_der = np.sqrt(balance_factor) * layer_config.amplitude
                    
                    audio_estructura[0, inicio_sample:fin_sample] += onda_layer * factor_izq
                    audio_estructura[1, inicio_sample:fin_sample] += onda_layer * factor_der
                    
                    # REGISTRAR CAPA GENERADA CON INFO REAL
                    capas_generadas.append({
                        "layer_index": i,
                        "config_real": {
                            "frequency": layer_config.frequency,
                            "amplitude": layer_config.amplitude,
                            ...
                            "wave_type": layer_config.wave_type.value,
                            "modulation": layer_config.modulation.value if layer_config.modulation else None,
                            "spatial_position": layer_config.spatial_position,
                            "phase_offset": layer_config.phase_offset
                        },
                        "timing_real": {
                            "inicio_sec": inicio_sample / self.sample_rate,
                            "fin_sec": fin_sample / self.sample_rate,
                            "duracion_sec": duracion_layer / self.sample_rate
                        },
                        "balance_estereo": balance_factor,
                        "samples_generados": len(onda_layer),
                        "motor_generador": "NeuroWaveGenerator"
                    })
                    
                except Exception as e:
                    error_msg = f"Error generando layer {i} con motor real: {e}"
                    logger.error(f"❌ {error_msg}")
                    raise RuntimeError(error_msg)
            
            # NORMALIZACIÓN CON PARÁMETROS REALES DEL MOTOR
            max_amplitude = np.max(np.abs(audio_estructura))
            if max_amplitude > 0:
                target_level = 0.8 if self.quality_level == 'high' else 0.7
                normalization_factor = target_level / max_amplitude
                audio_estructura = audio_estructura * normalization_factor
                logger.info(f"🔧 Normalización real aplicada: {normalization_factor:.3f} (target: {target_level})")
            
            # ACTUALIZAR ESTADÍSTICAS REALES DEL MOTOR
            processing_time = time.time() - start_time
            self.stats.layers_generados += len(layers_estructura)
            self.stats.templates_procesados += 1
            self.stats.tiempo_total_generacion += processing_time
            self.stats.detecciones_exitosas += 1
            self.stats.ultima_actualizacion = datetime.now()
            
            # RESULTADO CON METADATOS REALES COMPLETOS
            resultado = {
                "audio_data": audio_estructura,
                "sample_rate": self.sample_rate,
                "duracion_sec": duracion_sec,
                "objetivo": objetivo,
                "capas_generadas": capas_generadas,
                "numero_capas": len(capas_generadas),
                "layers_config_reales": layers_estructura,
                "componentes_detectados": componentes_detectados,
                "metadatos": {
                    "motor": self.nombre,
                    "version": self.version,
                    "metodo": "crear_estructura_capas",
                    "infraestructura_real_usada": {
                        "detector": "DetectorComponentesHyperMod",
                        "gestor": "GestorAuroraIntegradoV32",
                        "generador": "NeuroWaveGenerator",
                        "templates_detectados": len(templates_reales),
                        "componentes_disponibles": list(componentes_detectados.keys())
                    },
                    "tiempo_procesamiento": processing_time,
                    "configuracion_motor": {
                        "quality_level": self.quality_level,
                    ...
                    "estadisticas_motor_actualizadas": {
                        "layers_generados_total": self.stats.layers_generados,
                        "templates_procesados_total": self.stats.templates_procesados,
                        "tiempo_total_motor": self.stats.tiempo_total_generacion,
                        "detecciones_exitosas": self.stats.detecciones_exitosas
                    }
                }
            }
            
            logger.info(f"🏗️ ESTRUCTURA CREADA EXITOSAMENTE: {len(capas_generadas)} capas reales para '{objetivo}' en {processing_time:.2f}s")
            return resultado
            
        except Exception as e:
            error_msg = f"Error crítico creando estructura con motores reales: {e}"
            logger.error(f"❌ {error_msg}")
            # NO FALLBACK SILENCIOSO - REPORTAR ERROR CLARAMENTE
            raise RuntimeError(f"crear_estructura_capas FALLÓ usando infraestructura real: {error_msg}")

    def _crear_capas_cognitivas(self, configuracion: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Crea capas específicas para objetivos cognitivos"""
        return [
            {'tipo': 'binaural', 'frecuencia_base': 40, 'frecuencia_diferencia': 6, 'amplitud': 0.7},
            {'tipo': 'carrier', 'frecuencia': 256, 'amplitud': 0.5},
            {'tipo': 'harmonic', 'frecuencia': 528, 'amplitud': 0.3}
        ]

    def _crear_capas_relajacion(self, configuracion: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Crea capas específicas para relajación"""
        return [
            {'tipo': 'binaural', 'frecuencia_base': 60, 'frecuencia_diferencia': 4, 'amplitud': 0.8},
            {'tipo': 'pad', 'frecuencia': 136.1, 'amplitud': 0.6},
            {'tipo': 'ambient', 'frecuencia': 432, 'amplitud': 0.4}
        ]

    def _crear_capas_genericas(self, configuracion: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Crea capas genéricas"""
        return [
            {'tipo': 'base', 'frecuencia': 440, 'amplitud': 0.6},
            {'tipo': 'structural', 'frecuencia': 220, 'amplitud': 0.4}
        ]
