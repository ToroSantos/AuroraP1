    ...
    def _mezclar_capas_colaborativas_pesadas(self, capas_audio: List[np.ndarray], configuracion: ConfiguracionAuroraUnificada) -> Optional[np.ndarray]:
        ...
        try:
            ...
            for i, capa in enumerate(capas_seguras):
                if len(capa) < 1000:  # Menos de 1000 samples = inválida
                    self.logger.warning(f"⚠️ Capa {i} demasiado pequeña ({len(capa)} samples), saltando")
                    continue
            ...
            max_length = max(len(capa) for capa in capas_validas)
            ...
            for i, capa in enumerate(capas_validas):
                if len(capa) < max_length:
                    # Extender capa corta repitiendo contenido
                    repeticiones = (max_length // len(capa)) + 1
                    if len(capa.shape) == 1:
