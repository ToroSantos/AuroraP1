...
class AdaptadorFormatosAuroraV7:
    ...
    def _es_formato_hypermod_v32(self, salida: Any, nombre_motor: str) -> bool:
        ...
        return (isinstance(salida, np.ndarray) and 
                ('hypermod' in nombre_motor.lower() or 'hyper' in nombre_motor.lower() or 
                 'v32' in nombre_motor or 'structure' in nombre_motor.lower()))
    
    def _normalizar_hypermod_v32(self, salida: np.ndarray, context: str) -> Tuple[np.ndarray, Dict]:
        ...
        self.logger.debug(f"✅ HyperMod V32 normalizado: {salida.shape}")
        self._stats_conversion['success'] += 1
    ...
    def _es_formato_harmonic_v34(self, salida: Any, nombre_motor: str) -> bool:
        ...
        return (isinstance(salida, np.ndarray) and 
                ('harmonic' in nombre_motor.lower() or 'essence' in nombre_motor.lower() or 
                 'v34' in nombre_motor or 'texture' in nombre_motor.lower()))
    
    def _normalizar_harmonic_v34(self, salida: np.ndarray, context: str) -> Tuple[np.ndarray, Dict]:
        ...
        self.logger.debug(f"✅ HarmonicEssence V34 normalizado: {salida.shape}")
        self._stats_conversion['success'] += 1
...
class AuroraDirectorV7Refactored(DirectorAuroraInterface):
    ...
    def _generar_colaborativo_optimizado(self, configuracion: ConfiguracionAuroraUnificada, 
        ...
        try:
            ...
            if capas_colaborativas:
                ...
                return self._generar_audio_fallback(configuracion)
    ...
    def _mezclar_capas_inteligente(self, capas_audio: List[Any], configuracion: ConfiguracionAuroraUnificada) -> np.ndarray:
        """
        Mezcla inteligente de capas de audio con adaptador universal
        ✅ CORREGIDO: Compatibilidad total con formatos Aurora V7
        ✅ NUEVO: Uso de AdaptadorFormatosAuroraV7 para normalización automática
        """
        try:
            # ✅ NUEVO: Inicializar adaptador específico
            adaptador = AdaptadorFormatosAuroraV7(self.logger)
            capas_normalizadas = []
            
            self.logger.debug(f"🎵 Iniciando mezcla inteligente de {len(capas_audio)} capas")
            
            # ✅ NUEVO: Normalización específica por motor
            for i, capa_raw in enumerate(capas_audio):
                try:
                    # Detectar motor origen desde metadata si está disponible
                    nombre_motor = self._detectar_motor_origen(capa_raw, i)
                    
                    # Normalizar usando adaptador específico
                    capa_normalizada, metadata = adaptador.normalizar_salida_motor_real(
                        capa_raw, 
                        nombre_motor,
                        context="mezcla_inteligente"
                    )
                    ...
                    # Ajustar duración según configuración
                    duracion_objetivo = int(configuracion.duracion * configuracion.sample_rate)
                    capa_normalizada = self._ajustar_duracion_capa(capa_normalizada, duracion_objetivo)
                    
                    capas_normalizadas.append(capa_normalizada)
                    self.logger.debug(f"✅ Capa {i} ({metadata['motor_origen']}) normalizada: {capa_normalizada.shape}")
                    
                except Exception as e:
                    self.logger.error(f"❌ Error procesando capa {i}: {e}")
                    continue
            
            # ✅ RESTO DE LA LÓGICA ORIGINAL (preservada)
            if not capas_normalizadas:
                self.logger.warning("⚠️ No hay capas válidas para mezclar, generando audio de fallback")
                return self._generar_audio_silencio(configuracion)
            
            # Mezcla vectorizada optimizada
            return self._realizar_mezcla_vectorizada(capas_normalizadas, configuracion)
...
# ============================================================================
    ...
    def _mezclar_capas_colaborativas_pesadas(self, capas_colaborativas: List[Dict], 
                                        configuracion: ConfiguracionAuroraUnificada) -> np.ndarray:
        """Mezcla capas colaborativas con pesos específicos"""
        try:
            if not capas_colaborativas:
                return self._generar_audio_fallback(configuracion)
            
            # Encontrar dimensiones comunes
            max_length = max(len(capa['audio']) for capa in capas_colaborativas)
            num_channels = max(capa['audio'].shape[1] if len(capa['audio'].shape) > 1 else 1 
                            for capa in capas_colaborativas)
            
            # Inicializar audio mezclado
            audio_mezclado = np.zeros((max_length, num_channels))
            peso_total = 0
            
            # Mezclar cada capa con su peso
            for capa in capas_colaborativas:
                audio_capa = capa['audio']
                peso = capa['peso']
                
                # Ajustar formato
                if len(audio_capa.shape) == 1:
                    audio_capa = np.column_stack([audio_capa] * num_channels)
                
                # Ajustar longitud
                if len(audio_capa) < max_length:
                    padding = np.zeros((max_length - len(audio_capa), audio_capa.shape[1]))
                    audio_capa = np.vstack([audio_capa, padding])
                elif len(audio_capa) > max_length:
                    audio_capa = audio_capa[:max_length]
                
                # Añadir con peso
                audio_mezclado += audio_capa * peso
                peso_total += peso
                
                self.logger.debug(f"✅ Mezclado {capa['metadata']['motor_origen']} con peso {peso:.2f}")
            
            # Normalizar por peso total
            if peso_total > 0:
                audio_mezclado /= peso_total
            
            # Normalizar amplitud final
            max_val = np.max(np.abs(audio_mezclado))
            if max_val > 0.95:
                audio_mezclado *= 0.95 / max_val
            
            self.logger.info(f"🎵 Mezcla colaborativa completa: {len(capas_colaborativas)} motores, peso total: {peso_total:.2f}")
            return audio_mezclado
            
        except Exception as e:
            self.logger.error(f"❌ Error en mezcla colaborativa pesada: {e}")
            return self._generar_audio_fallback(configuracion)
