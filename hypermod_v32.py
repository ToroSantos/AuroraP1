    ...
    def _crear_fallback_objective_templates(self) -> Dict[str, Any]:
        ...
        fallback_templates.update({
            'benchmark': {
                ...
                'description': 'Template optimizado para benchmarks de rendimiento'
            },
            'test': {
                'name': 'Test Template',
...
class DetectorComponentesHyperMod:
    ...
    def _detectar_estilos_audio(self) -> Optional[Dict[str, Any]]:
        """Detecta estilos de audio disponibles desde field_profiles"""
        try:
            # Busca estilos en múltiples ubicaciones
            estilos_encontrados = {}
            
            # Intenta importar field_profiles
            import field_profiles

            # Prioridad 1: PERFILES_CAMPO_AURORA
            if hasattr(field_profiles, 'PERFILES_CAMPO_AURORA'):
                estilos_encontrados = field_profiles.PERFILES_CAMPO_AURORA

            # Prioridad 2: FIELD_PROFILES
            elif hasattr(field_profiles, 'FIELD_PROFILES'):
                estilos_encontrados = field_profiles.FIELD_PROFILES

            if estilos_encontrados:
                self.stats_deteccion['estilos_detectados'] = len(estilos_encontrados)
                logger.info(f"✅ Detectados {len(estilos_encontrados)} estilos de audio desde field_profiles")
                return estilos_encontrados
            
        except Exception as e:
            logger.debug(f"⚠️ Error al detectar estilos desde field_profiles: {e}")
    ...
    def _detectar_fases_estructurales(self) -> Optional[Dict[str, Any]]:
        """Detecta fases estructurales con detección robusta"""
        try:
            import presets_fases
            if hasattr(presets_fases, 'GestorFasesConscientes'):
                gestor = presets_fases.GestorFasesConscientes()
                fases = gestor.fases_base
                self.stats_deteccion['fases_detectadas'] = len(fases)
                logger.info(f"✅ Detectadas {len(fases)} fases estructurales REALES")
                return fases
            elif hasattr(presets_fases, 'FASES_ESTRUCTURALES'):
                fases = presets_fases.FASES_ESTRUCTURALES
                self.stats_deteccion['fases_detectadas'] = len(fases)
                logger.info(f"✅ Detectadas {len(fases)} fases estructurales desde FASES_ESTRUCTURALES")
                return fases
        except Exception as e:
            logger.warning(f"⚠️ Error detectando fases estructurales reales: {e}")
    
        # Fallback como último recurso
        logger.info("🔄 Usando fases estructurales de fallback")
        return self._crear_fases_fallback()
    ...
    def _detectar_templates_objetivos(self) -> Optional[Dict[str, Any]]:
        ...
        # Fallback como último recurso
        templates = self._crear_fallback_objective_templates()  # ✅ Corregido aquí
        logger.info("🔄 Usando templates de fallback")
        return templates
