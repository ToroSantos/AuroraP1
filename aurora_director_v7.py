    ...
    def _detectar_componente_factory(self, nombre: str, config: Dict[str, Any]) -> Optional[Any]:
        ...
        try:
            ...
            if tipo == TipoComponente.MOTOR:
                ...
                # ✅ CORRECCIÓN: Todos los gestores usan crear_gestor()
                ...
                # ✅ AGREGADO: Soporte para pipeline components
                return aurora_factory.crear_analizador(nombre)  # Los pipelines usan crear_analizador
            else:
...
class AuroraDirectorV7Refactored(ComponenteAuroraBase):
    ...
    def __init__(self):
        ...
        # Configuración básica
        self.modo_orquestacion = ModoOrquestacion.FALLBACK  # Inicia en modo seguro
        ...
        logger.info("🎯 Aurora Director V7 refactorizado inicializado (modo pasivo)")
    
    def inicializar_deteccion_completa(self):
        """
        Inicializa la detección completa del sistema (ÚNICO PUNTO DE DETECCIÓN)
        ...
        try:
            logger.info("🔍 Iniciando detección completa del sistema Aurora (PUNTO ÚNICO)")
            ...
            logger.info(f"✅ Detección completa exitosa: {len(self.motores)} motores, {len(self.gestores)} gestores")
            return True
    ...
    def _inicializar_componentes_detectados(self):
        """Inicializa manualmente los componentes que lo requieren"""
        
        # Inicializar NeuroMix Super si está disponible
        if "neuromix_super_unificado" in self.motores:
            motor_neuromix = self.motores["neuromix_super_unificado"]
            if hasattr(motor_neuromix, 'inicializar_deteccion_manual'):
                try:
                    exito = motor_neuromix.inicializar_deteccion_manual()
                    if exito:
                        logger.info("✅ NeuroMix Super inicializado manualmente")
                    else:
                        logger.warning("⚠️ NeuroMix Super falló en inicialización manual")
                except Exception as e:
                    logger.warning(f"⚠️ Error inicializando NeuroMix Super: {e}")
        
        # Aquí se pueden agregar otros componentes que requieran inicialización manual
        logger.debug("🔧 Inicialización manual de componentes completada")
    ...
    def crear_experiencia(self, config: ConfiguracionBaseInterface) -> ResultadoBaseInterface:
        ...
        """
        # Asegurar que el sistema esté detectado antes de generar
        if not self.sistema_detectado:
            logger.info("🔍 Sistema no detectado, inicializando detección...")
    ...
    def obtener_estado_completo(self) -> Dict[str, Any]:
        """
        Alias para obtener_estado_sistema() - Compatibilidad con aurora_bridge.py
