...
class NeuroMixSuperUnificadoRefactored(ComponenteAuroraBase):
    ...
    def _obtener_neuromix_base(self) -> Optional[Any]:
        """
        🔧 VERSIÓN CORREGIDA: Obtiene NeuroMix base con protección anti-circular
        
        PROBLEMA ANTERIOR:
        - Factory devolvía AuroraNeuroAcousticEngineV27Connected
        - Connected hereda de NeuroMixSuperUnificado
        - NeuroMixSuperUnificado llama _obtener_neuromix_base()
        - _obtener_neuromix_base() llama factory
        - BUCLE INFINITO: Factory → Connected → Super → Factory → ...
        
        SOLUCIÓN APLICADA:
        - Blacklist de nombres circulares
        - Detección de auto-referencia
        - Fallback directo cuando se detecta circularidad
        - Lazy loading con protección estricta
        """
        
        # 🚫 BLACKLIST ANTI-CIRCULAR: Nombres que indican derivados problemáticos
        nombres_prohibidos = [
            "Connected",      # AuroraNeuroAcousticEngineV27Connected
            "Super",          # NeuroMixSuperUnificado y derivados
            "Definitivo",     # neuromix_definitivo_v7 y derivados
            "Refactored",     # Versiones refactorizadas
            "Unificado"       # Cualquier versión unificada
        ]
        
        try:
            # VERIFICACIÓN 1: Verificar que factory no está en inicialización
            if hasattr(aurora_factory, '_initialization_in_progress') and aurora_factory._initialization_in_progress:
                logger.warning("⚠️ Factory en inicialización, evitando deadlock")
                return None
            
            # VERIFICACIÓN 2: Intentar usar factory con protección anti-circular
            try:
                neuromix_base = aurora_factory.crear_motor("neuromix_aurora_v27")
                
                if neuromix_base:
                    # PROTECCIÓN ANTI-CIRCULAR: Verificar nombre de clase
                    nombre_clase = type(neuromix_base).__name__
                    
                    # Detectar auto-referencia por nombre
                    if any(nombre_prohibido in nombre_clase for nombre_prohibido in nombres_prohibidos):
                        logger.warning(f"🚫 Auto-referencia detectada: {nombre_clase}")
                        logger.warning("   Contiene nombres prohibidos, usando fallback directo")
                        return None
                    
                    # PROTECCIÓN ADICIONAL: Verificar que no seamos nosotros mismos
                    if hasattr(neuromix_base, '__class__') and 'NeuroMixSuperUnificado' in str(neuromix_base.__class__.__mro__):
                        logger.warning("🚫 Herencia circular detectada en MRO")
                        logger.warning("   NeuroMixSuperUnificado en la cadena de herencia")
                        return None
                    
                    logger.info(f"✅ NeuroMix base seguro obtenido vía factory: {nombre_clase}")
                    return neuromix_base
                
            except RecursionError as re:
                logger.error(f"🚫 RECURSIÓN INFINITA DETECTADA en factory: {re}")
                logger.error("   Activando protección anti-Ouroboros")
                return None
                
            except Exception as e:
                logger.warning(f"⚠️ Error usando factory (posible circular): {e}")
            
            # VERIFICACIÓN 3: Fallback con lazy loading directo
            logger.info("🔧 Intentando lazy loading directo como fallback")
            
            try:
                # Importar directamente el motor base ORIGINAL (no derivados)
                import importlib
                neuromix_module = importlib.import_module('neuromix_aurora_v27')
                
                # BUSCAR CLASE BASE ORIGINAL: No las Connected o derivadas
                clases_disponibles = [attr for attr in dir(neuromix_module) if not attr.startswith('_')]
                
                for nombre_clase in clases_disponibles:
                    # Buscar clase que NO contenga nombres prohibidos
                    if not any(prohibido in nombre_clase for prohibido in nombres_prohibidos):
                        if 'AuroraNeuroAcousticEngine' in nombre_clase and 'V27' in nombre_clase:
                            clase_base = getattr(neuromix_module, nombre_clase)
                            
                            # Verificar que es una clase instanciable
                            if isinstance(clase_base, type):
                                neuromix_base = clase_base()
                                logger.info(f"✅ NeuroMix base obtenido vía lazy loading: {nombre_clase}")
                                return neuromix_base
                
                logger.warning("⚠️ No se encontró clase base segura en lazy loading")
                
            except Exception as e:
                logger.warning(f"⚠️ Error en lazy loading: {e}")
            
            # VERIFICACIÓN 4: Fallback usando lazy_importer si está disponible
            if hasattr(self, 'lazy_importer') and self.lazy_importer:
                try:
                    neuromix_class = self.lazy_importer.get_class('neuromix_aurora_v27', 'AuroraNeuroAcousticEngineV27')
                    if neuromix_class and not any(prohibido in neuromix_class.__name__ for prohibido in nombres_prohibidos):
                        neuromix_base = neuromix_class()
                        logger.info(f"✅ NeuroMix base obtenido vía lazy_importer: {neuromix_class.__name__}")
                        return neuromix_base
                except Exception as e:
                    logger.warning(f"⚠️ Error usando lazy_importer: {e}")
            
            # RESULTADO: Sin motor base disponible de forma segura
            logger.warning("⚠️ NeuroMix base no disponible de forma segura")
            logger.warning("   Todas las rutas presentan riesgo de circularidad")
            logger.warning("   Sistema funcionará en modo fallback puro")
            return None
            
        except Exception as e:
            logger.error(f"❌ Error crítico obteniendo NeuroMix base: {e}")
            return None
