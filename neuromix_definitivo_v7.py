        ...
        try:
            ...
            if hasattr(audio_data, 'shape'):
                ...
                if total_elements > 50_000_000:  # Más de 50M elementos = PELIGRO CRÍTICO
                    ...
                    logger.error("🚨 TRUNCANDO para prevenir explosión de memoria (255 TiB)")
                    ...
                    if len(audio_data.shape) == 2:
                        ...
                        else:  # Formato anómalo - truncar dimensión más grande
                            if audio_data.shape[0] > audio_data.shape[1]:
                                audio_data = audio_data[:max_samples, :]
                            else:
                                audio_data = audio_data[:, :max_samples]
                            logger.warning(f"✅ Audio con formato anómalo truncado por seguridad")
                            
                    elif len(audio_data.shape) == 1:  # Audio mono
                        max_samples = 44100 * 60
                        if len(audio_data) > max_samples:
                            audio_data = audio_data[:max_samples]
                            logger.warning(f"✅ Audio mono truncado a {max_samples} samples por seguridad")
                            
                    else:  # Dimensiones completamente anómalas
                        logger.error(f"❌ DIMENSIONES CRÍTICAS ANÓMALAS: {audio_data.shape}")
                        logger.error("🚨 Retornando respuesta de error por seguridad")
                        return {
                            "error": f"Dimensiones de audio peligrosas: {audio_data.shape}",
                            "audio_data": None,
                            "sample_rate": 44100,
                    ...
                    # Verificar que el truncado fue exitoso
                    new_total = np.prod(audio_data.shape)
                    logger.info(f"✅ Array seguro después del truncado: {audio_data.shape} = {new_total:,} elementos")
                    
                    # Actualizar resultado_potenciado con información de truncado
                    if isinstance(resultado_potenciado, dict):
                        resultado_potenciado["security_truncated"] = True
                        resultado_potenciado["original_shape_dangerous"] = True
                        resultado_potenciado["warning"] = "Audio truncado por seguridad de memoria"
                        
                # Validación adicional de dimensiones seguras
                if len(audio_data.shape) > 2:
                ...
                # Verificar que ninguna dimensión individual sea peligrosa
                    ...
                    if shape_1 == 2 and shape_0 > shape_1:
                        ...
                        try:
                            if len(envolvente) == shape_1:
                                envolvente_2d = np.row_stack([envolvente, envolvente])
                                audio_enriquecido = audio_data * envolvente_2d
                            else:
                                # Redimensionar envolvente
                                envolvente_resized = np.interp(
                                ...
                                envolvente_2d = np.row_stack([envolvente_resized, envolvente_resized])
                                audio_enriquecido = audio_data * envolvente_2d
                        except Exception as e:
