...
class ExtensionMultiMotor:
    ...
    def _obtener_director_disponible(self):
        """Intenta obtener un director Aurora disponible"""
        try:
            # Buscar en el sistema de módulos
            import sys
            if 'aurora_director_v7' in sys.modules:
                modulo_director = sys.modules['aurora_director_v7']
                
                # Buscar clases de director disponibles
                for nombre_clase in ['AuroraDirectorV7Refactored', 'AuroraDirectorV7Integrado', 'DirectorAurora']:
                    if hasattr(modulo_director, nombre_clase):
                        ClaseDirector = getattr(modulo_director, nombre_clase)
                        director = ClaseDirector()
                        logger.debug(f"✅ Director obtenido: {nombre_clase}")
                        return director
            
            return None
        except Exception as e:
            logger.debug(f"⚠️ No se pudo obtener director automáticamente: {e}")
            return None
