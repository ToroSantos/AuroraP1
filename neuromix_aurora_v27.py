...
try:
    ...
    if hasattr(neuromix_definitivo_v7, 'NeuroMixSuperUnificado'):
        class AuroraNeuroAcousticEngineV27Connected(neuromix_definitivo_v7.NeuroMixSuperUnificado):
            def __init__(self):
                super().__init__()
                self.version = "V27_AURORA_CONNECTED_REAL"
                self.enable_advanced = True
                
                # Inicializar logger para la corrección
            ...
            def get_processing_stats(self):
                ...
                if hasattr(super(), 'get_processing_stats'):
                    return super().get_processing_stats()
                return {
                    "total_generated": getattr(self, '_total_generated', 0),
                    "ppp_compatibility_uses": getattr(self, '_ppp_uses', 0),
                    "aurora_integrations": getattr(self, '_aurora_uses', 0)
                }
            
            def generar_audio(self, config, duracion_sec=None):
                """
                🔧 CORRECCIÓN DE RAÍZ: Método que ADAPTA las firmas incompatibles
                
                PROBLEMA REAL IDENTIFICADO:
                - NeuroMixSuperUnificado.generar_audio(configuracion) -> 1 parámetro
                - AuroraNeuroAcousticEngineV27.generar_audio(config, duracion_sec) -> 2 parámetros
                
                SOLUCIÓN: Unificar ambos parámetros en uno solo y llamar al super() correctamente.
                Es como traducir entre dos idiomas diferentes para que se entiendan.
                """
                try:
                    # Extraer duracion_sec del config si no se proporciona
                    if duracion_sec is None:
                        duracion_sec = config.get('duracion_sec', config.get('duracion_min', 60.0))
                        if isinstance(duracion_sec, str):
                            try:
                                duracion_sec = float(duracion_sec)
                            except:
                                duracion_sec = 60.0
                    
                    # 🎯 CORRECCIÓN DE RAÍZ: Preparar configuración unificada para NeuroMixSuperUnificado
                    # NeuroMixSuperUnificado espera TODO en un solo diccionario de configuración
                    configuracion_unificada = config.copy() if isinstance(config, dict) else {}
                    configuracion_unificada['duracion_sec'] = duracion_sec
                    
                    self.logger.info(f"✅ Configuración unificada preparada para super()")
                    self.logger.info(f"✅ Parámetros: {len(configuracion_unificada)} elementos, duracion_sec: {duracion_sec}")
                    
                    # ✅ USAR SUPER() CORRECTAMENTE: Solo pasar UN parámetro como espera NeuroMixSuperUnificado
                    resultado_super = super().generar_audio(configuracion_unificada)
                    
                    # Extraer audio_data del resultado de NeuroMixSuperUnificado
                    if isinstance(resultado_super, dict) and 'audio_data' in resultado_super:
                        audio_final = resultado_super['audio_data']
                        self.logger.info(f"✅ Audio extraído de NeuroMixSuperUnificado: {audio_final.shape if hasattr(audio_final, 'shape') else 'unknown shape'}")
                        return audio_final
                    elif hasattr(resultado_super, 'shape'):
                        # Si ya es un array de numpy directamente
                        self.logger.info(f"✅ Audio directo de NeuroMixSuperUnificado: {resultado_super.shape}")
                        return resultado_super
                    else:
                        self.logger.warning(f"⚠️ Resultado inesperado de super(): {type(resultado_super)}")
                        return resultado_super
                    
                except Exception as e:
                    # Log del error real con detalles completos
                    self.logger.error(f"❌ ERROR en generar_audio con super() corregido: {e}")
                    self.logger.error(f"❌ Config type: {type(config)}, duracion_sec: {duracion_sec}")
                    
                    # 🎯 FALLBACK INTELIGENTE: Si super() falla, usar motor directo
                    self.logger.info(f"🔄 Intentando fallback con motor directo...")
                    
                    try:
                        motor_directo = AuroraNeuroAcousticEngineV27(enable_advanced_features=True)
                        resultado_directo = motor_directo.generar_audio(config, duracion_sec)
                        self.logger.info(f"✅ Fallback exitoso con motor directo")
                        return resultado_directo
                    except Exception as e2:
                        self.logger.error(f"❌ Fallback también falló: {e2}")
                        # Re-lanzar el error original para diagnóstico
                        raise e
            
            def obtener_capacidades(self) -> Dict[str, Any]:
                """
                MÉTODO AGREGADO: Compatibilidad con Aurora Director V7
                Preserva funcionalidad original + interface Aurora
                """
                try:
                    # Intentar usar capacidades del super si existen
                    if hasattr(super(), 'obtener_capacidades'):
                        capacidades_base = super().obtener_capacidades()
                        # Agregar información específica de Aurora V27
                        capacidades_base.update({
                            "aurora_v27_connected": True,
                            "real_classification": True,
                            "super_unificado": True
                        })
                        return capacidades_base
                except Exception as e:
                    logger.warning(f"Error obteniendo capacidades del super: {e}")
                
                # Fallback: capacidades completas construidas manualmente
                return {
                    "nombre": "NeuroMix V27 Aurora Connected + PPP Complete",
                    "version": getattr(self, 'version', 'V27_AURORA_CONNECTED_REAL'),
                    "tipo": "motor_neuroacustico",
                    "compatible_con": ["Aurora Director V7", "PPP Functions", "Field Profiles", "Objective Router"],
                    "neurotransmisores_soportados": self.get_available_neurotransmitters(),
                    "tipos_onda": self.get_available_wave_types(),
                    "modos_procesamiento": ["legacy", "standard", "advanced", "parallel", "realtime", "aurora_integrated"],
                    "sample_rates": [22050, 44100, 48000],
                    "duracion_minima": 1.0,
                    "duracion_maxima": 3600.0,
                    "calidad_maxima": "therapeutic",
                    ...
                    "real_classification": True,
                    "super_unificado": True,
                    "estadisticas": self.get_processing_stats()
                }
            
            def validar_configuracion(self, config) -> bool:
                """
                MÉTODO AGREGADO: Requerido por Aurora Director V7
                Preserva funcionalidad original + interface Aurora
                """
                try:
                    # Intentar usar validación del super si existe
                    if hasattr(super(), 'validar_configuracion'):
                        return super().validar_configuracion(config)
                    
                    # Validación básica si no hay super
                    ...
                    duracion = config.get('duracion_min', config.get('duracion_sec', 20))
                    ...
                    logger.error(f"Error en validar_configuracion: {e}")
                    return False
            
            def generate_neuro_wave_advanced(self, config):
                """Generación avanzada compatible"""
                if hasattr(super(), 'generate_neuro_wave_advanced'):
                    return super().generate_neuro_wave_advanced(config)
                # Fallback básico
                audio = self.generar_audio(config.__dict__ if hasattr(config, '__dict__') else config, 2.0)
                return audio, {"quality_score": 85}
        
        # Asignar la clase Connected a todos los alias
        AuroraNeuroAcousticEngineV27 = AuroraNeuroAcousticEngineV27Connected
        
        # Crear instancia global con la clase Connected
        
        print("🚀 NeuroMix REAL Classification aplicado exitosamente")
    else:
        raise ImportError('NeuroMixSuperUnificado no disponible')
        
except ImportError:
    print("💡 NeuroMix Super no disponible, usando motor base")
    # Usar motor base existente (AuroraNeuroAcousticEngineV27 ya definido arriba)
    pass
        
except Exception as e:
    print(f"⚠️ Error integrando NeuroMix Super: {e}")
    # Motor base sigue disponible como fallback seguro

# ================================================================
# VERIFICACIÓN ADICIONAL: Asegurar compatibilidad _global_engine
# ================================================================

def asegurar_compatibilidad_global_engine():
    """Asegura que _global_engine tenga todos los métodos necesarios"""
    global _global_engine
    
    if not hasattr(_global_engine, 'obtener_capacidades'):
        # Agregar método dinámicamente si no existe
        def obtener_capacidades_fallback():
            return {
                "nombre": f"NeuroMix V27 Fallback - {getattr(_global_engine, 'version', 'unknown')}",
                "tipo": "motor_neuroacustico_fallback",
                "aurora_v27_connected": False,
                "fallback_mode": True,
                "message": "Usando capacidades de fallback"
            }
        
        _global_engine.obtener_capacidades = obtener_capacidades_fallback
        logger.info("🔧 Método obtener_capacidades() agregado dinámicamente")

# Aplicar verificación automáticamente
asegurar_compatibilidad_global_engine()
