...
# Reemplaza la función index() en tu aurora_flask_app.py (línea 471 aproximadamente)

@app.route('/')
def index():
    """Página principal con GUI completa o fallback"""
    try:
        # 🎯 PRIMERO: Intentar cargar el template HTML profesional
        template_path = Path(app.template_folder or 'templates') / 'index.html'
        
        if template_path.exists():
            logger.info("✅ Cargando GUI profesional desde templates/index.html")
            return render_template('index.html')
        else:
            logger.warning("⚠️ templates/index.html no encontrado, usando fallback")
            return index_fallback()
    
    except Exception as e:
        logger.error(f"❌ Error cargando GUI: {e}")
        return index_fallback()

def index_fallback():
    """Fallback HTML con información de motores detectados"""
    motores_info = ""
    ...
    <head>
        ...
        <style>
            * {{ margin: 0; padding: 0; box-sizing: border-box; }}
            ...
            .gui-upgrade {{ 
                background: linear-gradient(45deg, #FFE082, #FFA726); 
                padding: 20px; border-radius: 10px; margin: 20px 0; 
    ...
    <body>
        <div class="container">
            ...
            <div class="gui-upgrade">
                <h3>🎨 ¿Quieres la GUI Profesional?</h3>
                <p>Para usar la interfaz gráfica completa, crea el archivo:</p>
                <code><strong>templates/index.html</strong></code>
                <br><br>
                <p>📂 Asegúrate de que tengas:</p>
                <ul style="text-align: left; margin: 10px 0;">
                    <li>📁 <code>templates/index.html</code> - Interfaz HTML</li>
                    <li>📁 <code>static/css/aurora-core.css</code> - Estilos</li>
                    <li>📁 <code>static/js/aurora-core.js</code> - JavaScript</li>
                </ul>
            </div>
...
# Agrega esto a tu aurora_flask_app.py después de las otras rutas

@app.route('/api/debug/frontend')
def debug_frontend():
    """Debug específico para problemas de frontend"""
    try:
        ...
        debug_info = {
            ...
            'motores_detectados': {
                'total': len(MOTORES_REALES),
                'detalles': list(MOTORES_REALES.keys())
            },
            'aurora_bridge_status': 'ok' if aurora_bridge else 'error',
            'endpoints_disponibles': [
                '/',
                '/test', 
                '/api/health',
                '/api/system/status',
                '/api/detected-engines',
        ...
        # Test básico del Aurora Bridge
        if aurora_bridge:
            try:
                system_status = aurora_bridge.get_complete_system_status()
                debug_info['system_status_test'] = 'ok'
                debug_info['motores_activos'] = system_status.get('summary', {}).get('motores_activos', 0)
            except Exception as e:
                debug_info['system_status_test'] = f'error: {str(e)}'
        
        return jsonify(debug_info)
        
    except Exception as e:
        return jsonify({
            'error': 'Error en debug frontend',
            'message': str(e),
            'timestamp': datetime.now().isoformat()
        }), 500

@app.route('/api/gui/initial-state')  
def gui_initial_state():
    """Estado inicial optimizado para la GUI"""
    try:
        # Estado simplificado para inicialización rápida
        initial_state = {
            'system': {
                'status': 'operational',
                'timestamp': datetime.now().isoformat(),
            ...
            'motors': {
                'total': len(MOTORES_REALES),
                'active': len([m for m in MOTORES_REALES.values() if m.get('instancia_creada', False)]),
                'names': list(MOTORES_REALES.keys())[:3]  # Solo los primeros 3 para UI
            },
            'capabilities': {
                'generation': True,
            ...
            'ui': {
                'default_duration': 20,
                'available_styles': ['crystalline', 'sereno', 'organico'],
                'available_intensities': ['suave', 'media', 'intenso']
            }
        }
